...
新增审批授权页搜索功能优化
快打应用/快打审批埋点任务完成
新增线下性能测试场景用例，执行 v1.9 线下性能测试输出报告，结论：可正常上线
...
PcAllDetailPage: 'c_oa_p0rzve6o', // PC 端全部审批详情页，是否需要改过来，只包含快打审批详情页
...
7.3：技术方案 DOA，开发 DOA
7.4：选人组件涉及：1.0 前后台升级最新包（是否有上限？），2.0 后台条件组编辑，审批任务和抄送任务缺陷 【快打 2.0】后台-编辑权限组页面-添加成员选人组件，右侧面板不应该展示 20 上限 [已关闭]，修复之后，将最新的包进行升级
...
6.26：联调
6.27：tt 值班；快打没东西；开平；
6.17：修改 1.0 后台子应用路径支持泳道域名访问、三方审批菜单权限逻辑修复、周会、5 月质量复盘会、对齐性能测试细节问题
6.18：重新发布并安装审批流组件包，协助 qa 测试审批流组件报错渲染，开发开平
6.19：修复 bug，开发资源管理平台 0.7PD，0.2PD 看测试
6.20：完成快打线下性能卡控建立文档https://km.sankuai.com/collabpage/2313326614
、修复提测 bug、DOA 需求评审
6.21：开发开平
...
6.4：修复小语种问题上线、修复
6.5：坚哥 bugbash、kuaida-app 审批详情页部门展示优化成省略号、v0.5 开平技术方案 1h、产品走查缺陷 【快打 1.0】移动端或者侧边栏查看离职调岗流程的基本信息，发起部门展示空白（建议没有展示-或者不展示） [已关闭]、kuaida-app 升级了包，还未合入 release，shenpi-mobile 有 bugfix，还未合入 release
...
bugfix：缺陷 【MVP-V1.5】快打审批操作审批时表单字段校验失败没有锚定到错误位置 [已关闭]
开放平台 v0.4 迭代
开放平台-V0.4 管理后台优化
联调中，预计 5.23 提测
tt 值班
...
shenpiAdmin 使用审批域名时出现跨域问题，导致子应用加载不出来，不能用 / ，应该直接用 curShenpiUrl，斜杠会默认访问到快打域名
...
技术方案书写
**20240412**
4.12 hotfix 缺陷 【快打v1.0 线上】审批详情页审批说明中，鼠标移动到流程负责人姓名 vCard 上时，hover 出来的卡片和实际流程负责人不一致 [打开]
参加校验评审、开平后端技术方案
产品对齐入口融合
**20240411**
融合入口文档，确定选型方案
...
开发链接转卡片需求 50%
20240201
已修复，快打1.0，分支未命名，v1.3 上线：缺陷 【快打 1.0】流程导出数据过期后点击重新查询->查询并导出后，报错“pdId 必须大于 0” [已关闭]
已修复，fix/sign-border-0201 未上线：缺陷 【快打】Windows 上加签内边框展示不全 [已关闭]
tt 值班++
20240131
修复：缺陷 【MVP-V1.2】移动端：审批任务转交后，原审批人点击消息卡片上“审批详情”，进入详情页后点击页面左上角返回箭头页面刷新，预期是返回到审批公众号会话页 [已关闭]
排查 bug：缺陷 【快打】Windows 上加签内边框展示不全 [已关闭]
20240130
校招生述职
20240129
写述职文档
...
，及发布等问题
记录：快打v 1.0 发布需要到 next 上专门部署才能生效
20230913
批量编辑添加选项条数校验
...
20230824
开发单选人员当前展示逻辑，包含快打 1.0&2.0&PC&移动单选人员控件优化
PC 端单据转交新增转交理由【https://km.sankuai.com/collabpage/1803578615#b-01764705a6a44de19d337e298da8547d
】
明日计划：
梳理设计器结构，为 radio 批量添加功能铺垫
20230823
梳理单选人员当前展示逻辑，包含快打v 1.0&2.0&PC&移动https://km.sankuai.com/collabpage/1832667811
移动端单据转交列表页样式修改
明日计划：
开发单选人员组件优化问题
20230822
快打 1.0&2.0 移动端驳回意见提交页开发【完成】
优化并统一快打 1.0&2.0 移动端审批详情页 Toast 展示样式【完成】
明日计划：
规划单据转交功能移动端并开发
总结：
npm 包上传学习
...
**20230704**
熟悉 快打 -> 审批中心 -> 大象 整个链路运行规则，快打一体化审批搭建平台使用手册
...
快打迭代 SOP：版本负责人按照值班表迭代，主要负责需求文档的创建、render 版本的发布
梳理项目从开发到部署的具体操作流程
快打迭代 SOP
开发前、开发中
