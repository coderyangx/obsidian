...
// ** 通过 getSelection() 实现文本高亮显示，获取选中文字
{
...
console.log('获取到的 selection', selection, selection.toString(), range.cloneContents());

var span = document.createElement('span');

span.style.background = 'pink'

range.surroundContents(span); // 跨节点选中会报错，要求所选内容必须完全包含在一个节点内
...
// this.attachShadow({ mode: "open" }) // 创建一个 shadow root，可以实现css隔离
}
// 当元素被插入到DOM中时调用
connectedCallback() {
